...
本 skill 假设 **Python >= 3.12** 且 **nexusx >= 3.2**。特性-版本对照：
...
> [!IMPORTANT]
> **Python 版本要求：`>= 3.12`。**
> 该 skill 依赖的 `SQLModel + SQLAlchemy + nexusx ErManager` 关系解析，在较低 Python 版本（尤其是 3.10）下，容易因为类型注解延迟求值与 ORM relationship 解析组合而出现兼容问题。
> 如使用 `Relationship(...)`、自引用关系、虚拟实体（`ErManager.add_virtual_entities`）或 Alembic 自动迁移，请统一使用 Python 3.12+。

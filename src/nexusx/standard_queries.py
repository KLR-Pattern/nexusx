...
def _create_by_keys_in_page_query(
    entity: type[SQLModel],
    session_factory: Any,
    field_name: str,
    field_type: type,
) -> Any:
    """Create a ``by_<field>_in_page`` paginated batch root for federation.
    ...
    ``total_count`` via ``COUNT(*) OVER`` (always computed; the mounter/executor
    filters pagination fields by the client selection, mirroring local
    ``pagination_selection``).

    Generated by default alongside ``by_<key>_in`` (zero-config) — see
    ``add_standard_queries``. The mounter decides which root to call based on
    whether a ``RemoteRelationship`` declared ``sort_field``.
    """

    arg_name = f"{field_name}_list"
    method_name = f"by_{field_name}_in_page"

    @query
    async def by_field_in_page(cls, **kwargs: Any) -> Any:
        ...
        # pk column for a stable tie-break ordering (matches local page loader).
        pk_fields = _get_primary_key_fields(cls)
        pk_name = pk_fields[0][0] if pk_fields else sort_field

        async with session_factory() as session:
            fk_col = getattr(cls, field_name)
            sort_col = getattr(cls, sort_field)
            sort_col = sort_col.desc() if sort_direction == "desc" else sort_col.asc()
            pk_col = getattr(cls, pk_name)

            rn_label = "_nx_rn"
            tc_label = "_nx_tc"
            row_num_col = func.row_number().over(
                partition_by=fk_col, order_by=[sort_col, pk_col],
            ).label(rn_label)
            total_count_col = func.count().over(partition_by=fk_col).label(tc_label)

            inner = select(cls, row_num_col, total_count_col).where(fk_col.in_(values))
            subq = inner.subquery()

            rn_col = subq.c[rn_label]
            fk_col_sub = subq.c[field_name]
            sort_col_sub = subq.c[sort_field]
            pk_col_sub = subq.c[pk_name]
            ...
            outer = (
                ...
                .order_by(fk_col_sub, sort_col_sub, pk_col_sub)
            )
            ...
            missing = [v for v in values if v not in total_counts]
            if missing:
                count_q = (
            ...
            for v in values:
                ...
                packages.append({
                    ...
                    "pagination": Pagination(
                        has_more=has_more, total_count=total_counts.get(v, 0),
                    ),
                })
            return packages

    func_obj = (
        by_field_in_page.__func__
        if hasattr(by_field_in_page, "__func__")
        else by_field_in_page
    )
    ...
    func_obj.__annotations__["sort_field"] = str
    func_obj.__annotations__["sort_direction"] = str
    by_field_in_page.__annotations__["return"] = list[dict]
    func_obj.__signature__ = inspect.Signature(
        parameters=[
            ...
            inspect.Parameter(
                "sort_field", inspect.Parameter.POSITIONAL_OR_KEYWORD,
                annotation=str,
            ),
            inspect.Parameter(
            ...
            inspect.Parameter(
                "sort_direction", inspect.Parameter.POSITIONAL_OR_KEYWORD,
                default="asc", annotation=str,
            ),
        ],
...
def add_standard_queries(
    ...
    for entity in entities:
        ...
        # The paginated root (by_<key>_in_page) is generated by default alongside
        # (zero-config): the mounter decides which to call based on whether its
        # RemoteRelationship declared sort_field. Unused roots are inert.
        for field_name in config.batch_keys.get(entity.__name__, []):
            method_name = f"by_{field_name}_in"
            page_method_name = f"by_{field_name}_in_page"
            ...
            if not hasattr(entity, page_method_name):
                setattr(
                    entity,
                    page_method_name,
                    _create_by_keys_in_page_query(entity, session_factory, field_name, field_type),
                )

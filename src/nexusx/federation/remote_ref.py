...
def _remote_ref_cardinality(annotation: Any) -> tuple[RemoteRef | None, bool]:
    ...
    if ref is None:
        return None, False
    # Peel Annotated (e.g. Annotated[list[Ref], Paged(...)] → list[Ref]) so
    # the list/union inspection sees the real container, not the Annotated
    # generic. specs/016 Phase 2: Paged marker on a remote-DTO field.
    inner = annotation

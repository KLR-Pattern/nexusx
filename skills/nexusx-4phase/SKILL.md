...
- 新项目默认逐阶段展示验收结果；用户明确要求端到端执行或现有项目增量迭代时，可连续完成多个阶段并在交付时集中汇报
- Phase 间递进：同一项目目录下逐步丰富，允许修改前一阶段文件，但不得破坏已通过的验收项
...
│   写入 specs/<编号>-<需求>/phaseN.md 的"验收标准"部分 │
└──────────────────────────────────────────────┘
                      ↓
              ┌───────────────┐
              │   实现 Phase   │

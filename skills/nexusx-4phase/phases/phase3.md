...
- `service/<domain>/dtos.py` — DefineSubset DTOs
- `service/<domain>/service.py` — UseCaseService
- `main.py` — 挂载用户选定的 REST / JSON-RPC / MCP / CLI / Compose GraphQL + Voyager

**关键模式**:
- `DefineSubset` + `SubsetConfig` 定义响应 DTO（字段选择、FK 隐藏）
- **nexusx 6.x 的 `DefineSubset.__subset__` 源可以是任意 `pydantic.BaseModel`**（不限于 SQLModel）—— 如果 Phase 0 Step 0-3 选了虚拟实体根（`CurrentUser`、`Page[T]`、第三方 SDK DTO 等），用同一语法从 BaseModel 源字段中选子集。SQLModel 源可由 ORM entity 转换，BaseModel 源由业务代码直接构造 DTO 实例（详见 `docs/guide/virtual_entities.md`）
- `AutoLoad` 标记 DTO 关系字段为自动加载（配合 Resolver implicit auto-load 使用，显式声明自动加载意图）
- **跨层数据流**：当 DTO 子节点需要祖先字段，或祖先需要聚合后代字段时，使用三个 helper（详见 `docs/api/api_cross_layer.md`）：
  - `ExposeAs(alias)` — 将当前字段以 alias 暴露到后代的 `ancestor_context`
  - `SendTo(collector_name)` — 将当前字段值发送给祖先的同名 Collector
  - `Collector(alias, flat=False)` — 在祖先 `post_*` 方法中收集后代发送的值
  - 请求上下文注入是独立机制：UseCaseService 参数使用 `Annotated[T, FromContext()]`
...
- **UseCaseService 方法必须声明返回类型注解**（如 `-> list[ChatSummary]`、`-> ChatSummary | None`），compose schema 生成器会强校验，缺注解直接在启动期报错
- **UseCaseService 复用 `service/<domain>/methods.py` 中的核心逻辑，不重新实现**：
  ...
  - **service.py 默认不直接操作数据库**（无 `async_session`）。确需按 DTO 裁剪宽表列时，在数据访问层使用 `build_dto_select(DtoType, where=...)`，并通过 `dict(row._mapping)` 构造 DTO
- **`build_dto_select(DtoType, where=None)` 查询构建** — 根据 DTO 的 `__subset__` 源实体和字段生成 SQL SELECT，只查询 DTO 需要的标量列
...
- **`create_use_case_graphql_mcp_server()` — UseCase GraphQL MCP**：4 层渐进披露，Layer 3 (`compose_query`) 接收标准 GraphQL 查询字符串：
...
- **UseCase 出口形态**（由用户在 Phase 3 决定取舍，按需启用）：
  | 出口 | 入口 | 适用场景 |
  |------|------|---------|
...
2. **DTO 字段类型必须用 DTO 类型** — 不能直接用 SQLModel 实体，compose schema 生成器会主动报 `SQLModelInDtoFieldError`
...
7. **使用 nexusx 6.x 支持的 FastMCP 版本范围** — 优先安装 `nexusx[fastmcp]>=6,<7`，不要单独声明冲突的 FastMCP 版本。挂载到 FastAPI 时使用 `transport="streamable-http", stateless_http=True`，并把 MCP http_app lifespan 合并到 FastAPI lifespan
...
9. **UseCaseService 方法必须声明返回类型注解** — compose schema 生成器（`build_compose_schema`）强校验返回类型；同时 `create_use_case_router()` 也会用返回类型生成 `response_model`
...
14. **UseCase MCP 使用 GraphQL 模式**（`create_use_case_graphql_mcp_server`）。旧的直接调用式 MCP 已移除，迁移见 `docs/migrations/3.0-use-case-graphql.md`。Layer 3 (`compose_query`) 接收标准 GraphQL 字符串，同时拒绝内省以保持 MCP 响应紧凑
